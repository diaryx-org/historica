...
- `MaterialiseError` gains `ForgottenPayload { payload, by, length }`, which
  is answered where `MissingPayload` is answered. 0067 landed alongside this
  and moved that place: `content_at` names a payload rather than reading one,
  so the question is no longer "what is the content here" but "why can this
  replica not produce these bytes" — asked by whoever wanted them, of
  `Store::absent_payload`, which is where the two halves are told apart.
  `update` refuses the file and names the `drop`.
