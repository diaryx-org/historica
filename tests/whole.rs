...
fn a_forgotten_payload_is_two_headers_and_a_store_that_still_checks() {
    ...
    // What a person asking for the photograph is told: not "not here yet",
    // which is what an undelivered payload says, but who destroyed it and how
    ...
    assert_eq!(
        held,
        fs::read(corpus().join("operations/02-photo.png")).expect("the payload")
    );
