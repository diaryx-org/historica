...
def check_mutations(temporary):
    ...
    mutations = (
        ...
        (
            ...
            "prune_lemmas.cleared_derived",
            "prune.bend",
        ),
        (
            "prune clears a file of cache/ that is no file",
            '      Rev.keep(Bool.and(String.starts_with(l, "f "), T.is_digest(entry_name(l))), entry_name(l), cached(rest))',
            '      Rev.keep(T.is_digest(entry_name(l)), entry_name(l), cached(rest))',
            "prune_lemmas.cleared_derived",
            "prune.bend",
        ),
        (
            "prune clears cache/ by a name outside it",
            '  T.map(~String, ~String, ~(n => "cache/" ++ n), cached(String.lines(text)))',
            '  T.map(~String, ~String, ~(n => n), cached(String.lines(text)))',
            "prune_lemmas.cleared",
            "prune.bend",
        ),
        (
