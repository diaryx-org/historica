...
def check_store(temporary):
    ...
    def compare(corpus, commands):
        ...
        for command in commands:
            ...
            writes = command[0] in ("record", "name", "init", "update", "merge", "forget", *REWRITES, *MOVES)
...
def check_mutations(temporary):
    ...
    mutations = (
        ...
        (
            ...
            "receive_lemmas.here_ok",
            "receive.bend",
        ),
        (
            "receive joins two unrelated histories unasked",
            "      plan.related(Bool.or(join, related(side.fulls(here), side.fulls(there))), here, there)",
            "      plan.related(True{}, here, there)",
            "receive_lemmas.source_ok",
            "receive.bend",
        ),
        (
        ...
        (
            ...
            "Bool.pick(List<&2, Act>, unrecorded(held, p <> heads), [Act.Said{UNRECORDED(at)}], [Act.Whole{at, from, p, m}])",
            "Bool.pick(List<&2, Act>, False{}, [Act.Said{UNRECORDED(at)}], [Act.Whole{at, from, p, m}])",
            "merging_lemmas.whole_ok",
            "merging.bend",
        ),
        (
