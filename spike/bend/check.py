...
def check_mutations(temporary):
    ...
    mutations = (
        ...
        (
            ...
            "editor_lemmas.unsaved",
            "editor.bend",
        ),
        (
            "an editor that would not start is not named",
            'Fail{Stop{1, editor ++ ": " ++ msg}}',
            "Fail{Stop{1, msg}}",
            "Laws.editor_that_would_not_start_is_said",
            "editor.bend",
        ),
        (
            "the editor's file is read back where it did not save",
        ...
        (
            ...
            "Some{checked(a, None{})}",
            "identity_lemmas.env_unset",
            "identity.bend",
        ),
        (
            "the refusal of $HISTORICA_AUTHOR does not name it",
            '"`" ++ who ++ "` in $HISTORICA_AUTHOR cannot be an author:',
            '"`" ++ who ++ "` cannot be an author:',
            "identity_lemmas.env_refused",
            "identity.bend",
        ),
        (
            "a set $HISTORICA_AUTHOR still reads the identity file",
            "    case Some{r}:\n      IO.pure(Result<&2, &2, String, String>, r)",
            "    case Some{r}:\n      author.file(directory)",
            "identity_lemmas.env_first",
            "identity.bend",
        ),
        (
        ...
        (
            ...
            "      +found = rest",
            "Laws.check_finds_nothing_in_a_new_store",
            "check.bend",
        ),
        (
            "init writes its header where the store does not look for it",
            '    [Note{"/historica.txt", Notes.HEADER_NOTE()}, Note{"/skipped/README.txt", Notes.SKIPPED_NOTE()},',
            '    [Note{"/historica", Notes.HEADER_NOTE()}, Note{"/skipped/README.txt", Notes.SKIPPED_NOTE()},',
            "Laws.check_finds_nothing_in_a_new_store",
            "commands.bend",
        ),
        (
            "init writes a note beside its store rather than in it",
            '     Note{"/format.txt", Notes.FORMAT_NOTE()}, Note{"/cache/README.txt", Notes.CACHE_NOTE()}]}',
            '     Note{"format.txt", Notes.FORMAT_NOTE()}, Note{"/cache/README.txt", Notes.CACHE_NOTE()}]}',
            "init_lemmas.only_its_store",
            "commands.bend",
        ),
        (
            "init writes a note into a directory it does not make",
            '    ["/revisions", "/operations", "/names", "/cache", "/skipped"],',
            '    ["/revisions", "/operations", "/names", "/skipped"],',
            "init_lemmas.only_its_store",
            "commands.bend",
        ),
        (
            "init writes its notes where it runs rather than under the store",
            "      Note{root ++ p, t} <> init.under.notes(root, rest)",
            "      Note{p, t} <> init.under.notes(root, rest)",
            "init_lemmas.notes_under",
            "commands.bend",
        ),
        (
            "the cache note loses the newline its last line ends with",
            '    "a little time and no information."\n  ], "\\n") ++ "\\n"',
            '    "a little time and no information."\n  ], "\\n")',
            "Laws.init_writes_text",
            "notes.bend",
        ),
        (
            "a note ends a line with a carriage return",
            '    "You may delete any or all of it; Historica will rebuild what it needs.",',
            '    "You may delete any or all of it; Historica will rebuild what it needs.\\r",',
            "Laws.init_writes_text",
            "notes.bend",
        ),
        (
            "an absolute path is joined under the base",
            '  Bool.pick(String, String.starts_with(p, "/"), p,',
            '  Bool.pick(String, String.starts_with(p, "/"), base ++ p,',
        ...
        (
            "the validator refuses 9F after ED",
            "U32.is_eq(lead, 237), v.in(b, 128, 159)",
            "U32.is_eq(lead, 237), v.in(b, 128, 158)",
            "utf8_lemmas.ed.second",
            "utf8.bend",
        ),
        (
            "the validator refuses BF as a continuation byte",
            "Bool.and(U32.is_ge(b, 128), U32.is_le(b, 191))",
            "Bool.and(U32.is_ge(b, 128), U32.is_le(b, 190))",
            "utf8_lemmas.cont6",
            "utf8.bend",
        ),
        (
            "the validator refuses F4 as a lead byte",
            "Bool.pick(Nat, v.in(b, 240, 244), 4n, 0n)",
            "Bool.pick(Nat, v.in(b, 240, 243), 4n, 0n)",
            "utf8_lemmas.width4",
            "utf8.bend",
        ),
        (
            "the validator refuses 90 after F0",
            "U32.is_eq(lead, 240), v.in(b, 144, 191)",
            "U32.is_eq(lead, 240), v.in(b, 145, 191)",
            "utf8_lemmas.f0.second.a",
            "utf8.bend",
        ),
        (
        ...
        (
            ...
            '      beside(path, leaf(stem) ++ ".rev.txt")',
            '      "revisions/" ++ stem ++ ".rev.txt"',
            "arrange_lemmas.kept_here",
            "arrange.bend",
        ),
        (
            "arrange renames a revision out of the folder it sits in",
            "      Bool.pick(String, has_slash(SCon{c, rest}), SCon{c, beside(rest, name)}, name)",
            "      Bool.pick(String, has_slash(rest), SCon{c, beside(rest, name)}, name)",
            "arrange_lemmas.beside_slashed",
            "arrange.bend",
        ),
        (
            "arrange files a revision under its stem's month in place",
            "      Bool.pick(String, has_slash(rest), leaf(rest), Bool.pick(String, Char.is_eq(c, '/'), rest, SCon{c, rest}))",
            "      Bool.pick(String, False{}, leaf(rest), Bool.pick(String, Char.is_eq(c, '/'), rest, SCon{c, rest}))",
            "arrange_lemmas.leaf_flat",
            "arrange.bend",
        ),
        (
        ...
        (
            "arrange's real run leaves out a file whose name filled",
            "      Came{rs, a, List.append(&2, T.Split, os, [T.Split{from, to}]), u}",
            "      Came{rs, a, os, u}",
            "arrange_lemmas.push_tally",
            "arrange.bend",
        ),
        (
        ...
        (
            "prune reads every revision as superseded",
            "      Bool.or(Rev.member(supersedes_of(k), id), superseded(rest, id))",
            "      Bool.or(True{}, superseded(rest, id))",
            "prune_lemmas.superseded_is",
            "prune.bend",
        ),
        (
            "prune's report names digests, not the files it removes",
            '      said.listed(pruned.files(p), Bool.pick(String, dry, "would remove", "removed"))',
            '      said.listed(pruned.all(p), Bool.pick(String, dry, "would remove", "removed"))',
            "prune_lemmas.dry_run",
            "prune.bend",
        ),
        (
            "prune removes every revision document",
            "      Tree.keep(~T.Split, Arrange.has(dead, id), T.Split{path, id}, gone.revs(rest, dead))",
            "      Tree.keep(~T.Split, True{}, T.Split{path, id}, gone.revs(rest, dead))",
            "prune_lemmas.revs_go",
            "prune.bend",
        ),
        (
            "prune reads no payload a revision names by `bytes` as needed",
            'List.append(&2, String, fact_digests(Tree.facts_of(facts, "text")), fact_digests(Tree.facts_of(facts, "bytes"))))',
            'fact_digests(Tree.facts_of(facts, "text")))',
            "prune_lemmas.names_false",
            "prune.bend",
        ),
        (
            "prune removes a forgetting document standing in for named content",
            "      Rev.keep(Arrange.has(named, t), id, rest)",
            "      Rev.keep(False{}, id, rest)",
            "prune_lemmas.forgetting_step",
            "prune.bend",
        ),
        (
            "prune clears every file of cache/",
            "      Rev.keep(T.is_digest(x), x, rest)",
            "      x <> rest",
            "cleared_lemmas.file_cleared",
            "prune.bend",
        ),
        (
            "prune clears a file of cache/ that is no file",
            '      Bool.pick(Maybe<&2, String>, String.eq(kind, "f"), file.sized(T.split_once(rest)), None{})',
            "      file.sized(T.split_once(rest))",
            "cleared_lemmas.dir_cleared",
            "prune.bend",
        ),
        (
            "prune reads a link's target in cache/ as an entry",
            '      digest.push(file.line(T.split_once(l)), cached(rest, String.starts_with(l, "l ")))',
            "      digest.push(file.line(T.split_once(l)), cached(rest, False{}))",
            "cleared_lemmas.link_cleared",
            "prune.bend",
        ),
        (
            "prune clears cache/ by a name outside it",
            '  T.map(~String, ~String, ~(n => "cache/" ++ n), cached(String.lines(text), False{}))',
            '  T.map(~String, ~String, ~(n => n), cached(String.lines(text), False{}))',
            "cleared_lemmas.cleared",
            "prune.bend",
        ),
        (
            "prune removes the documents of revisions it keeps",
            "  pruned.of(gone.revs(revs, Set.from_list(Rev.without(Main.ids(fs), Main.ids(k)))), gone.of(",
            "  pruned.of(gone.revs(revs, Set.from_list(Main.ids(fs))), gone.of(",
            "prune_lemmas.may_go",
            "prune.bend",
        ),
        (
        ...
        (
            ...
            "receive_lemmas.lacking_fresh",
            "receive.bend",
        ),
        (
            "receive copies a payload this store already holds",
            "      Tree.keep(~T.Split, Bool.not(Arrange.has(have, split.digest(s))), s, splits.lacking(rest, have))",
            "      Tree.keep(~T.Split, True{}, s, splits.lacking(rest, have))",
            "receive_lemmas.slacking_fresh",
            "receive.bend",
        ),
        (
            "receive takes a revision this store already holds",
            "  Set.from_list(Main.ids(side.fulls(here)))",
            "  Set.from_list(Nil{})",
            "receive_lemmas.revisions",
            "receive.bend",
        ),
        (
            "receive files this store's own revisions as the source's",
            "    docs.lacking(side.revisions(there), have.revisions(here)),",
            "    docs.lacking(side.revisions(here), have.revisions(here)),",
            "receive_lemmas.revisions",
            "receive.bend",
        ),
        (
        ...
        (
            "receive reads no supersedes edge as relating two histories",
            "      List.append(&2, String, List.append(&2, String, Main.parents_of(f), Prune.supersedes_of(f)), edges(rest))",
            "      List.append(&2, String, Main.parents_of(f), edges(rest))",
            "receive_lemmas.edges_is",
            "receive.bend",
        ),
        (
        ...
        (
            "export lays two paths of the tree out as one",
            "Placed{Tree.entry_path(e), [e]} <> Placed{path, es} <> rest)",
            "Placed{path, e <> es} <> rest)",
            "placed_lemmas.pp_push",
            "export.bend",
        ),
        (
            "export carries a document of this store with its text dropped",
            "Some{Store.Doc{p, i, n, t}}, doc.find(rest, id))",
            "Some{Store.Doc{p, i, n, \"\"}}, doc.find(rest, id))",
            "export_lemmas.found_among",
            "export.bend",
        ),
        (
            "export updates a copy somebody recorded in",
            "Bool.pick(Maybe<&2, String>, Arrange.has(here, id), stranger(rest, here), Some{id})",
            "Bool.pick(Maybe<&2, String>, Arrange.has(here, id), stranger(rest, here), None{})",
            "onto_lemmas.stranger_done",
            "export.bend",
        ),
        (
            "export --files-only names its links among the files it wrote",
            "paths.said(\"wrote  \", put.paths(file.puts(ps)))",
            "paths.said(\"wrote  \", put.paths(ps))",
            "dryrun_lemmas.folder_said",
            "export.bend",
        ),
        (
        ...
        (
            "fetch finds a revision the two sides share only where all are shared",
            "      Bool.or(Rev.member(ys, x), any.in(rest, ys))",
            "      Bool.and(Rev.member(ys, x), any.in(rest, ys))",
            "fetchplan_lemmas.not_both",
            "fetch.bend",
        ),
        (
            "fetch joins unrelated histories unasked",
            "Bool.or(join, related(here.revs(h), digests.of(of.kind(os, \"revision\"))))",
            "Bool.or(True{}, related(here.revs(h), digests.of(of.kind(os, \"revision\"))))",
            "fetchplan_lemmas.refuses",
            "fetch.bend",
        ),
        (
            "fetch destroys every original it holds",
            "        Main.sorted_distinct(among.list(forgotten, List.append(&2, String, docs, pays)))}",
            "        Main.sorted_distinct(List.append(&2, String, docs, pays))}",
            "fetchplan_lemmas.destroys_held",
            "fetch.bend",
        ),
        (
            "fetch names a revision it took twice",
            "        Main.said.lines(\"revision \", SNil{}, Main.sorted_distinct(rs)),",
            "        Main.said.lines(\"revision \", SNil{}, List.sort(~String, ~(a => b => String.is_le(a, b)), rs)),",
            "fetchplan_lemmas.says_what_it_took",
            "fetch.bend",
        ),
        (
        ...
        (
            ...
            "update_lemmas.settled_iff",
            "update.bend",
        ),
        (
            "update will not say the folder holds the target while it leaves a file alone",
            ...
            "      Bool.and(Bool.and(Bool.and(List.is_empty(&2, Decided, writes(ds)), List.is_empty(&2, String, removes(ws))), Bool.and(List.is_empty(&2, Decided, modes(ds)), List.is_empty(&2, Decided, links(ds)))), List.is_empty(&2, String, leaves(ws)))",
            "update_lemmas.settled_iff",
            "update.bend",
        ),
        (
        ...
        (
            "update, with nothing to do, forgets the files it leaves alone",
            '      List.append(&2, String, said.leaves(leaves(ws)), ["the folder already holds " ++ spelled])',
            '      ["the folder already holds " ++ spelled]',
            "update_lemmas.settled_says",
            "update.bend",
        ),
        (
        ...
        (
            ...
            "update_lemmas.dry_side",
            "update.bend",
        ),
        (
            "update sets no mode its plan says to",
            "effects.links(links(ds)), effects.modes(modes(ds))])",
            "effects.links(links(ds))])",
            "update_lemmas.eff_side",
            "update.bend",
        ),
        (
        ...
        (
            ...
            "merging_lemmas.lines_at",
            "merging.bend",
        ),
        (
            "merge writes a file it sets aside at the path its first claimant keeps",
            "  +at = at.of(bs, e)",
            "  +at = Tree.entry_path(e)",
            "merging_lemmas.placed",
            "merging.bend",
        ),
        (
            "merge lays out only the first file of the merged tree",
            "      place(fs, ds, heads, bs, dg, ls, texts, e) <> places(fs, ds, heads, bs, dg, ls, texts, rest)",
            "      [place(fs, ds, heads, bs, dg, ls, texts, e)]",
            "merging_lemmas.placed_all",
            "merging.bend",
        ),
        (
        ...
        (
            "merge labels a run in words the marker search does not look for",
            '  "vvv historica: " ++ (String.take(name_at(names, a), 8n) ++ " wrote vvv")',
            '  "vvv historica " ++ (String.take(name_at(names, a), 8n) ++ " wrote vvv")',
            "merging_lemmas.label_marks",
            "conflict.bend",
        ),
        (
            "the marker search counts a line the merged file itself holds",
            "Bool.pick(List<&2, String>, Bool.and(is_marker(l), Bool.not(any_eq(plain, l))), l <> later, later)",
            "Bool.pick(List<&2, String>, is_marker(l), l <> later, later)",
            "merging_lemmas.not_marker",
            "conflict.bend",
        ),
        (
        ...
        (
            "record resolves the parent's lines rather than the folder's",
            "      resolved.content(f, p, Conflict.resolve(j, after))",
            "      resolved.content(f, p, Conflict.resolve(j, before_of(befores, f)))",
            "record_lemmas.lines_res",
            "commands.bend",
        ),
        (
            "merge lays a payload without the mode its tree names",
            "[Act.Whole{at, from, p, m}]",
            "[Act.Whole{at, from, p, Tree.Plain{}}]",
            "merging_lemmas.whole_at",
            "merging.bend",
        ),
        (
        ...
        (
            "forget --fields states only the first digest gone",
            '      ("gone " ++ t) <> said.gone(rest)',
            '      ("gone " ++ t) <> Nil{}',
            "forgetting_lemmas.gone_reads",
            "forget.bend",
        ),
        (
            "forget --dry-run says it destroyed what it would destroy",
            'said.destroys(ds, Bool.pick(String, dry, "would destroy", "destroyed"))',
            'said.destroys(ds, "destroyed")',
            "forgetting_lemmas.done_read",
            "forget.bend",
        ),
        (
            "a payload's stand-in is filed beside it whatever is there",
            'Bool.pick(String, filed.taken(fs, d ++ ".ops.txt"), named, d ++ ".ops.txt")',
            'd ++ ".ops.txt"',
            "forgetting_lemmas.payload_where",
            "forget.bend",
        ),
        (
        ...
        (
            ...
            "forgetting_lemmas.whole_refused",
            "forget.bend",
        ),
        (
            "forget's sweep removes operations/ itself",
            "Rev.keep(Bool.and(Bool.not(String.is_empty(dir)), String.is_empty(text)), dir, found)",
            "Rev.keep(String.is_empty(text), dir, found)",
            "forgetting_lemmas.sweeps",
            "forget.bend",
        ),
        (
        ...
        (
            "forget destroys the text of the item after the one picked",
            "      forget.item(picked.has(ps, target, i, j), it) <> forget.items(rest, ps, target, i, 1n+j)",
            "      forget.item(picked.has(ps, target, i, 1n+j), it) <> forget.items(rest, ps, target, i, 1n+j)",
            "forget_lemmas.items_at",
            "forget.bend",
        ),
        (
            "forget looks up the documents revisions state for another path",
            "      named.of(Main.id_of(f), Main.said(Main.facts_of(f), file), Main.is_text(Main.facts_of(f), file), named(rest, file))",
            "      named.of(Main.id_of(f), Main.said(Main.facts_of(f), \"x\"), Main.is_text(Main.facts_of(f), file), named(rest, file))",
            "forget_lemmas.named_stated",
            "forget.bend",
        ),
        (
        ...
        (
            ...
            "forget_lemmas.rd_dry",
            "forget.bend",
        ),
        (
            "forget's sweep reads a link's target as a directory",
            "    case l <> rest True{}:\n      swept.dirs(dir, rest, False{})",
            "    case +l <> rest True{}:\n      Rev.keep(String.starts_with(l, \"d \"), swept.join(dir, String.drop(l, 2n)), swept.dirs(dir, rest, False{}))",
            "forgetting_lemmas.past_link",
            "forget.bend",
        ),
        (
            "forget's sweep descends into a file",
            'Rev.keep(String.starts_with(l, "d "), swept.join(dir, String.drop(l, 2n))',
            'Rev.keep(Bool.or(String.starts_with(l, "d "), String.starts_with(l, "f ")), swept.join(dir, String.drop(l, 2n))',
            "forgetting_lemmas.dirs_named",
            "forget.bend",
        ),
        (
            "a revision refused for a path not in normal form C names the wrong line",
            'path ++ ": line " ++ Nat.show(n) ++ ": `" ++ p ++ "` is not a path',
        ...
        (
            ...
            "name_lemmas.nw.at_other",
            "commands.bend",
        ),
        (
            "name reads `--private` as `--shared`",
            "      NameCmd{p, Some{True{}}, d, f, List.append(&2, String, sh, [w]), r}",
            "      NameCmd{p, Some{False{}}, d, f, List.append(&2, String, sh, [w]), r}",
            "name_lemmas.nw.at_private",
            "commands.bend",
        ),
        (
            "name takes `--change` for a word it does not know",
            'Bool.pick(NWord, String.eq(w, "--change"), NWord.Change{},',
            'Bool.pick(NWord, String.eq(w, "--chnage"), NWord.Change{},',
            "name_lemmas.nw.word_other",
            "commands.bend",
        ),
        (
        ...
        (
            "name sets the bookmark at the head whatever target it is given",
            "    f : Full <- name.resolved.r(fs, bs, spelling)",
            '    f : Full <- name.resolved.r(fs, bs, "head")',
            "name_lemmas.points_ok",
            "commands.bend",
        ),
        (
        ...
        (
            ...
            "listing_lemmas.resolution_ok",
            "commands.bend",
        ),
        (
            "names cuts a pin to eight characters, unique or not",
            '      Bool.pick(String, Rev.member(all_ids, id), abbreviate(id, all_ids), "(not here yet)")',
            '      Bool.pick(String, Rev.member(all_ids, id), String.take(id, 8n), "(not here yet)")',
            "listing_lemmas.resolution_ok",
            "commands.bend",
        ),
        (
            "names abbreviates a change's revision against no other",
            "    case Rev.Resolved{+id}:\n      abbreviate(id, all_ids)",
            "    case Rev.Resolved{+id}:\n      abbreviate(id, Nil{})",
            "listing_lemmas.change_ok",
            "commands.bend",
        ),
        (
            "names says nothing where there are no bookmarks",
            '      ["no bookmarks here yet"]',
            "      Nil{}",
            "listing_lemmas.said_begun",
            "commands.bend",
        ),
        (
            "names leaves one space after the widest name",
            '      (pad(Bm.name_of(b), width) ++ "  " ++ Bm.show(b)',
            '      (pad(Bm.name_of(b), width) ++ " " ++ Bm.show(b)',
            "listing_lemmas.lines_ok",
            "commands.bend",
        ),
        (
        ...
        (
            "a bookmark's name keeps the slash after names/",
            "  +below = String.drop(path, 6n)",
            "  +below = String.drop(path, 5n)",
            "listing_lemmas.name_back",
            "bookmark.bend",
        ),
        (
            "a bookmark's name keeps the dot of .txt",
            "      +bare = String.take(n, Nat.sub(String.length(n), 4n))",
            "      +bare = String.take(n, Nat.sub(String.length(n), 3n))",
            "listing_lemmas.name_back",
            "bookmark.bend",
        ),
        (
            "names finds no bookmark under names/",
            "      Named{n, path} <> rest",
            "      rest",
            "listing_lemmas.files_at",
            "commands.bend",
        ),
        (
            "names reads only the first file under names/",
            "      named.of(p, Bm.name(p), named(rest))",
            "      named.of(p, Bm.name(p), Nil{})",
            "listing_lemmas.named_files",
            "commands.bend",
        ),
        (
        ...
        (
            "log counts an added file's bytes as stored",
            '   Figure{"stored", count_facts(facts, "bytes", added, True{})}]',
            '   Figure{"stored", count_facts(facts, "bytes", added, False{})}]',
            "listing_lemmas.figures_counted",
            "commands.bend",
        ),
        (
        ...
        (
            "a resolution's `keep` left out of what it keeps",
            "    case Piece.Keep{d, f, c} <> rest:\n      d <> keeps.of(rest)\n",
            "    case Piece.Keep{d, f, c} <> rest:\n      keeps.of(rest)\n",
            "fetch_lemmas.pieces_ok",
            "resolution.bend",
        ),
        (
        ...
        (
            ...
            "    case True{}:\n      Nil{}\n",
            "fetch_lemmas.edit_at",
            "commands.bend",
        ),
        (
            "diff over the folder fetching what a `text` names for an `edit`",
            "      edit_of.is(d, String.eq(k, \"edit\"))\n",
            "      edit_of.is(d, String.eq(k, \"text\"))\n",
            "fetch_lemmas.stated_same",
            "commands.bend",
        ),
        (
            ...
            "    case Some{Side.Whole{d, n}}:\n      Nil{}\n    case _:\n      Nil{}\n",
            "fetch_lemmas.side_mem",
            "commands.bend",
        ),
        (
            "a payload diff shows asked about twice",
            "  distinct(List.sort(~String, ~(a => b => String.is_le(a, b)), payloads_of(cs)))\n",
            "  List.sort(~String, ~(a => b => String.is_le(a, b)), payloads_of(cs))\n",
            "fetch_lemmas.payloads_once",
            "commands.bend",
        ),
        (
