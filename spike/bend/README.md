...
| `LAWS.bend` / `PROOF.bend` | a hundred and eighty-seven claims about the code, each proven | the test suite and Verus replay helpers |
...
`names` lists every bookmark once, in name order, each line beginning with
its name, and says where a pin or a change resolves as the abbreviation of
the revision it names — one no other digest the store holds begins with —
or why there is none; `log --fields` lists, by its whole digest, only
...
A hundred and eighty-one mutations cover the primitive helpers, lost inserts, a lost trailing
...
`--revision` pinning the change, and `--fields`' header said twice; and thirteen
