...
| `LAWS.bend` / `PROOF.bend` | sixty-nine claims about the code, each proven | the test suite and Verus replay helpers |
...
| `blamed_lemmas.bend` | `blame_reads_the_walk`: `blame <target> <path>`'s rows, read without their authors, are the file the walk of the target's ancestry reads | `blame`'s attribution |
...
Sixty-two mutations cover the primitive helpers, lost inserts, a lost trailing
...
break: rows read from every element the walk placed, removed or not. The
proof gate rejects
