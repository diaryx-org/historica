...
| `LAWS.bend` / `PROOF.bend` | three hundred and seventy-six claims about the code, each proven | the test suite and Verus replay helpers |
...
| `fulls_lemmas.bend` | `revisions_read_are_their_documents`: every revision a command reads is one a document of `revisions/` spells, under the digest it is known by, byte for byte as `Rev.write` writes what the revision says — or, where a path it states is not in normal form C, with the refusal naming that path and line in place of its facts and the rest what the mended document says; `revisions_read_lose_no_document`: every document of `revisions/` that reads whole so is read | `Store::revisions` |
...
byte (`revisions_read_are_their_documents`), and every document there that
reads as a revision is read (`revisions_read_lose_no_document`).
...
(`forget_files_a_payloads_standin_where_it_was`), and the other versions
...
copy of what went (`forget_clears_every_copy`); the note and the
catalogues stay (`forget_clears_only_copies`). Last, every directory under `operations/`
...
naming the file and the digest (`forget_rewrites_only_the_files_documents`); a file of bytes loses the
...
named by a digest (`forget_clears_every_copy`) and never the note `init`
writes or a catalogue (`forget_clears_only_copies`); and its arguments are
...
a payload's stand-in beside the payload wherever that name is free
(`forget_files_a_payloads_standin_where_it_was`);
...
Four hundred and sixty-four mutations cover the primitive helpers, lost inserts, a lost trailing
