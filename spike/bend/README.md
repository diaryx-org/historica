...
| `LAWS.bend` / `PROOF.bend` | eighty-seven claims about the code, each proven | the test suite and Verus replay helpers |
...
| `check_lemmas.bend` | `check_pairs_each_payload`: `check` reports each payload with the digest asked for it — one per payload in the listing's order, none taken by a revision or an operation document | `check` |
...
Seventy-seven mutations cover the primitive helpers, lost inserts, a lost trailing
...
log-argument break: `--author` read as `--grep`; and one check break: a
document taking a payload's digest. The
proof gate rejects
