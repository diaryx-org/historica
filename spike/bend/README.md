...
| `offer_lemmas.bend` | `offer_names_no_private_bookmark`: the manifest is the one a store with no private bookmark would have; `offer_names_no_private_rule`: and the one a store with no private rule would have; `offer_names_each_file_by_its_digest`: every line names a file of the store's listing by the digest the listing gives it, at the label `fetch` files the line's path under, drawn from the part of the listing its kind says, and every payload, document, revision and file of `claims/`, every rule not `private` and every bookmark not private whose file the listing holds, has its line; `offer_lists_forgetting_as_prune_reads_it`: what the manifest says a document forgets is what `prune`, `receive` and `export` read it as forgetting, save a resolution the Rust tool's catalogue does not parse; `offer_lines_read_back`: `fetch`'s line reader reads each line it writes back as the file it names, whatever spaces the path holds | `offer` |
...
| `fetching_lemmas.bend` | `fetch_asks_under_the_manifests_directory`: a URL it accepts is the manifest's directory, ending with `/`, and a name with no `/` in it, put back together; `fetch_asks_only_for_what_the_manifest_names`: every path a pass asks the host for is a path of the manifest; `fetch_files_only_text_that_hashes_to_its_line`: a document is filed only where the text that arrived hashes to the digest its line gave; `fetch_lands_a_file_only_where_it_hashes_to_its_line`: a file that arrives whole is moved in from where it was staged only where the host found its bytes to hash to that digest; `fetch_files_a_payload_under_what_it_hashes_to`: and a payload is filed under the digest its bytes have; `fetch_writes_a_bookmark_once_a_pass`: whatever steps a pass takes and whatever the host answers for each, of what the pass lands, step by step with its state threaded through, no landing that writes a bookmark is followed by another that writes it | `fetch` |
...
| `nfc_lemmas.bend` | `nfc_leaves_ascii`: a path of ASCII is its own normal form C; `nfc_orders_marks_by_class`: canonical ordering leaves no mark after one of a higher class with no starter between; `nfc_ordering_keeps_every_mark`: and the marks of every class are the ones it was given, in order; `nfc_class_misses_nothing`: the class lookup, stopping at the first run past a character, finds what a search of every run finds over the crate's table; `nfc_leaves_separators_alone`: over the crate's tables, normal form C's algorithm makes of code points with a `/` or a `.` among them what it makes of those before, the separator, and what it makes of those after; `a_name_that_cannot_be_spelled_is_refused`: the host's lines for any directory, read back, refuse each name that is not UTF-8 once, where it is on disk, in the Rust tool's words, and nothing else | `format::nfc`, `working::walk` |
| `walked_lemmas.bend` | `the_walk_yields_a_path_once`: each file the walk yields is at a path after the one before it and not the same; `the_folder_spells_what_is_opened`: the name a read opens, read back from the host's lines for any directory, is the last entry name whose normal form is the path — never a link's target, never a name that is not UTF-8 — or the path itself where no entry's is | `working::walk`, decision 0033 |
...
| `merging.bend`, `merging_lemmas.bend` | `merge`: the heads joined — what is named, and every current head that is not — and laid out in the folder: each file of lines as the walk reads it, fenced where concurrent work met, each file of bytes as the payload the tree kept, each link where it points, and a file holding work nobody recorded left alone; then the `record` line that records it, built as the words a person types. `merge_joins_held_revisions`: every revision it joins is one the store holds; `merge_joins_every_head`: whatever a person names, every current head is joined, through a lemma that a sort by digest keeps every item under each digest; `merge_rewrites_nothing_merged`: where the trees contest nothing and the folder already holds the merge, every file it writes it writes with the bytes already there, and it points no link; `merge_counts_what_it_fences`, `merge_counts_nothing_unfenced`: a label or closing line it renders, standing on a line of its own that the walk's reading does not hold, is counted by `status.marked.of`, the search `status` and `record --merge` read a file with, and the walk's reading unfenced is counted nowhere; `merge_prints_the_record_that_records_it`: the words of the `record` line it prints, read by `record`'s own parser, are a merge of every head it joined as the person typed it, settling each file set aside at the path it wrote it; `merge_writes_in_place_or_marked_aside`: file by file over the merged tree, every act it lays out that writes stands at the file's own path, unless a contest over a path names the file after its first claimant, and then at a name holding `(historica `, the first eight characters of the file's identifier and `)`, read with `String.contains`; a file it writes carries the tree's mode, and every file of the tree is laid out once. It does not say two names set aside differ: two identifiers sharing their first eight characters are given one | `cli::record::merge`, `conflict::render` |
...
| `identity_lemmas.bend` | `identity_reads_back`: the file `identity` writes reads back as its author for work in any directory, wherever that author is one a revision can hold; `identity_takes_the_environment_first`, `identity_refuses_the_environment_by_name`, `identity_reads_the_file_without_the_environment`: `$HISTORICA_AUTHOR` naming one a revision can hold is the author, with no file read; set, not empty and naming one no revision can hold, the refusal names the variable and the value, with no file read; and only unset or empty does reading the identity file answer; `identity_file_is_where_the_rust_tool_looks`, `identity_makes_the_files_directory`: the file is where `identity_path` puts it, and the directory `identity` makes is the one it is in; `identity_reads_its_blocks`: a file of blocks reads back as the default author and every directory's, in order; and nine laws, `identity_refuses_…`, one for each refusal the Rust tool makes of a file, each after any such blocks and at the line it names; `editor_file_is_in_the_temporary_directory`: the editor's file sits directly in `$TMPDIR` or `/tmp` | `record::identity`, `identity` |
| `editor_lemmas.bend` | `editor_is_the_one_chosen`: the editor is `$VISUAL`, `$EDITOR` only where `$VISUAL` is not set, and none where the one deciding is empty; `editor_is_read_only_when_it_saved`, `editor_stops_without_saving`, `editor_that_would_not_start_is_said`: the file it gave the editor is read back where the editor exited 0; any other exit stops with 1 and `the editor stopped without saving`, and one that would not start stops with 1 and its name and the host's message, nothing read either way | `cli::from_an_editor` |
...
differently, each resolution it states, assembled against the store's
documents as `cat` assembles one, is the file of lines the folder holds,
wherever those documents mint what the walk's proposal stands under
...
and that `<dir>` is joined as `Path::join` joins it: an absolute `/d`
makes `/d/history` wherever `init` runs
(`init_takes_an_absolute_dir_as_it_is`), a `/` after `<dir>` is not
doubled (`init_doubles_no_slash`), and `.` is not resolved
(`init_keeps_a_dot`).
...
Who records is `$HISTORICA_AUTHOR` where it is set and not empty
(`identity_takes_the_environment_first`), refused by name where it names
one no revision can hold (`identity_refuses_the_environment_by_name`),
with no file read either way; only where it is unset or empty is it what
the identity file (`identity_reads_the_file_without_the_environment`)
...
the file is read back only where the editor exited 0
(`editor_is_read_only_when_it_saved`); one that stops with any other code
or a signal stops the command, saying it stopped without saving
(`editor_stops_without_saving`), one that would not start is named with
what the host said (`editor_that_would_not_start_is_said`), and one set
but empty is a usage
...
Every line names a file of the store's listing by the digest the listing
gives it: where `fetch` files the line's path, the part of the listing
its kind is drawn from holds that label with that digest, and every
payload, document, revision and file of `claims/` has its line, as does
every rule not `private` and every bookmark not private whose file the
listing holds (`offer_names_each_file_by_its_digest`), so a fetcher checking a file
against its line checks it against what the store holds; and what a document's line says it forgets is what `prune`,
...
pass is held for the rest of it, whatever the host answers and whatever
the steps between, so a second line naming it writes nothing
(`fetch_writes_a_bookmark_once_a_pass`, over `step.land` and the state
each step leaves, threaded as `steps.io` threads it); and nothing is asked for that the
...
(`nfc_class_misses_nothing`). Over the crate's tables the algorithm
takes `/` and `.` as walls: what it makes of code points with one among
them is what it makes of those before, the separator, and what it makes
of those after (`nfc_leaves_separators_alone`), which is why normalising
a path a name at a time normalises the whole of it; a string below
U+0300, which the quick check hands back unread, is not what that law is
about. Of the walk: each file it yields is at a path after the one
before it and not the same (`the_walk_yields_a_path_once`); a read opens
the last name the directory's listing, read back from the host's lines,
gives whose normal form is the path — a link's own name, never its
target, never a name that is not UTF-8 — or the path itself where there
is none (`the_folder_spells_what_is_opened`);
...
file's end left uncounted; and fifteen merge breaks: the current heads nobody
named left out, a head joined by its spelling rather than the revision it
names, a file written at its identifier rather than its path, a file of
lines or a payload written without its tree's mode, `record --merge`
resolving the parent's lines rather than the folder's, the file that keeps a
path written beside it too, a file set aside written at the path its
first claimant keeps, a file set aside named by the path rather than
its identifier, only the first file of the tree laid out, a link
...
typed, and a path settled there the other way round; and three breaks of a stand-in beside a held original:
...
layout; and nine identity breaks: the default author forgotten, an empty
`$HISTORICA_AUTHOR` taken for an author, its refusal not naming it, a set
one still reading the identity file, an empty
...
at its last line, and a directory's second block taken; and five editor
breaks: an empty editor run, an editor a signal ended taken to have saved,
one that would not start not named, its file read back where it did not
save, and its file put a directory down; and six more forgetting breaks:
