...
| `LAWS.bend` / `PROOF.bend` | a hundred and forty-three claims about the code, each proven | the test suite and Verus replay helpers |
...
A hundred and thirty-seven mutations cover the primitive helpers, lost inserts, a lost trailing
...
read; and nineteen writing breaks: `record` keeping the first `--onto`
...
and an `abandon` dry run naming the run in reverse. The
proof gate rejects
