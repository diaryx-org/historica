...
| `LAWS.bend` / `PROOF.bend` | a hundred and eighty-two claims about the code, each proven | the test suite and Verus replay helpers |
...
three characters decompose to, cannot. Two names a filesystem tells apart
that are one path are one file, the one the walk met last. And
...
the name, between the entries around it. Three stores hold all of it to
...
given decomposed paths, 32 commands; `unnormal`, a revision restated by
...
A hundred and seventy-one mutations cover the primitive helpers, lost inserts, a lost trailing
...
and an `abandon` dry run naming the run in reverse.; and six normal-form
